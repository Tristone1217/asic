...
property assertion1;
//    @(posedge sys_clk) (state == TWO) |-> (pi_money == 1'b1);
    @(posedge sys_clk) (state == IDLE) && (pi_money == 1'b1) |=> (state==ONE);
endproperty

assert  property(assertion1)
    $display("assertion worked", $time);
//else
//    $display("assertion don't work", $time);
